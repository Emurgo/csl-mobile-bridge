import {
  ...
  BigNum,
  Credential,
  decode_metadatum_to_json_str,   encode_json_str_to_metadatum,
...
async function generateRegistration(request:  {
...
const test: () => void = async () => {
  ...
  const address = await RewardAddress.new(
    0,
    await Credential.from_keyhash(
      await (await stakePrivateKey.to_public()).hash(),
    ),
  );

  const signer = async (hashedMetadata: Uint8Array | undefined) => {
    if (hashedMetadata == null) {
  ...
  const result = txMetaData.metadata()!;
