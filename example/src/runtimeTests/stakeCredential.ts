...
const test = async () => {
  ...
  const stakeCredBytes = stakeCred.to_bytes();
  const stakeCredFromBytes = await Credential.from_bytes(
    Buffer.from(stakeCredBytes),
  );
  assert(
  ...
  assert(
    Buffer.from(
      stakeCredFromBytes.to_keyhash()!.to_bytes(),
  ...
  const stakeCredFromScriptHash = await Credential.from_scripthash(
    scriptHash,
  );
  assert(
  ...
  assert(
    Buffer.from(
      stakeCredFromScriptHash.to_scripthash()!.to_bytes(),
    ).toString('hex') === keyHashHex,
