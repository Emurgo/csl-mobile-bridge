...
#[allow(non_snake_case)]
#[no_mangle]
pub unsafe extern "C" fn Java_io_emurgo_rnhaskellshelley_transactionMetadatumNewInt(env: JNIEnv, _: JObject, int_ptr: JRPtr) -> jobject {
  handle_exception_result(|| { 
    let int_jrptr = int_ptr.rptr(&env)?;
    let int = int_jrptr.typed_ref::<Int>()?;
    let result = TransactionMetadatum::new_int(int);
    result.rptr().jptr(&env)
  })
  .jresult(&env)
}
